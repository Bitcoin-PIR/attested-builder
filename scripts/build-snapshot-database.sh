...
#   ROOTS_ONLY        1 to keep only Merkle roots, root payload, evidence,
#                     quote, and small metadata. This is the TEE proof mode:
#                     it does not stage a server-loadable DB. Default 0.
...
write_roots_only_manifest() {
    ...
    {
        printf '# Auto-generated by attested-builder roots-only mode; do not hand-edit.\n'
        printf '# This is NOT a server-loadable database manifest.\n'
        printf '\n'
        printf '[manifest]\n'
        printf 'version = 1\n'
        printf 'kind = "roots-only"\n'
...
run_step 05-build-bucket-merkle "${bucket_merkle_args[@]}"

remove_roots_only_files \
    "$OUT_DIR/utxo_chunks_index_nodust.bin" \
    "$OUT_DIR/utxo_chunks_nodust.bin" \
    "$OUT_DIR/top100_addresses.bin" \
    "$OUT_DIR/whale_addresses.txt" \
    "$OUT_DIR/batch_pir_cuckoo.bin" \
    "$OUT_DIR/chunk_pir_cuckoo.bin" \
...
remove_roots_only_files "$OUT_DIR/utxo_set.bin"
...
run_step 09-build-onion-merkle "${onion_merkle_args[@]}"
...
remove_roots_only_files \
    ...
    "$OUT_DIR/merkle_onion_sib_index.bin" \
    "$OUT_DIR/merkle_onion_sib_data.bin"
...
if is_truthy "$STAGE_SERVER_DB"; then
    ...
    printf 'server_db_dir=%s\n' "$SERVER_DB_DIR" | tee -a "$SUMMARY"
    printf 'server_db_manifest=%s\n' "$SERVER_DB_DIR/MANIFEST.toml" | tee -a "$SUMMARY"
    printf 'server_db_manifest_kind=roots-only\n' | tee -a "$SUMMARY"
    printf 'server_db_manifest_sha256=%s\n' "$(hash_one "$SERVER_DB_DIR/MANIFEST.toml")" | tee -a "$SUMMARY"
fi
