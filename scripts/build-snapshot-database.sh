    ...
    (
        ...
        find . -type f -print | LC_ALL=C sort |
            while IFS= read -r path; do
                case "$path" in
                    ./logs/*|*.manifest.sha256|*.manifest.diff|./build-summary.txt|./build.env|./build-receipt.txt)
                        continue
                        ;;
                esac
...
write_manifest "$OUT_DIR" "$DATABASE_MANIFEST" database
write_manifest "$OUT_DIR" "$ALL_ARTIFACTS_MANIFEST" all
